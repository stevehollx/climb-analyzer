...
export async function DELETE() {
  try {
    const dataDir = path.join(process.cwd(), '..', 'data', 'planet_osm_data');

    const { deleted, errors } = await deleteDirectoryContents(dataDir);

    if (errors.length > 0) {
      return NextResponse.json({
        message: `Deleted ${deleted} OSM files with ${errors.length} errors`,
        deleted,
        errors
      });
    }
    ...
    return NextResponse.json({
      message: `Successfully deleted ${deleted} OSM files`,
      deleted
    });
  } catch (error) {
    console.error('Failed to delete OSM data:', error);
    return NextResponse.json(
      { error: 'Failed to delete OSM data', details: String(error) },
      { status: 500 }
    );
  }
}
...
export async function GET() {
  try {
    ...
    console.error('Failed to get OSM data info:', error);
    return NextResponse.json(
      { error: 'Failed to get OSM data info', details: String(error) },
      { status: 500 }
    );
  }
}
